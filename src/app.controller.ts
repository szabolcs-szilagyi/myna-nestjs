    ...
    switch (part) {
      ...
      case PartOption.SetAddressData:
        return this.forwardGot
          ...
          .then(({ body }) =>
            res.json({ success: (<any>body).success ? '1' : '0' }),
          );

      case PartOption.SetSessionToken:
        return this.forwardGot
          .get('token/session', { isStream: true })
          .pipe(res);
      ...
      case PartOption.GetProductsNumberInCart:
        return this.forwardGot
          ...
          .then(({ statusCode, body }) => {
            if (statusCode < 300) {
              return res.json({
                nr: (<CartEntity[]>(<unknown>body)).length.toString(),
              });
            } else if (statusCode < 500) {
              throw new BadRequestException();
            } else {
